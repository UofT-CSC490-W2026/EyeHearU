...
parser.add_argument("--save-tag", type=str, default=None, help="model tag for saving checkpoints (defaults to --model-tag)")
# Training horizon
...
# Reward weights (so components are separable later)
parser.add_argument("--w-correct", type=float, default=1.0, help="weight for base correctness reward")
parser.add_argument("--w-format", type=float, default=0.2, help="weight for format reward (GSM8K-style answer)")
parser.add_argument("--w-steps", type=float, default=0.2, help="weight for reasoning-steps reward")
parser.add_argument("--w-close", type=float, default=0.3, help="weight for close-arithmetic reward")
args = parser.parse_args()
user_config = vars(args).copy()
# -----------------------------------------------------------------------------


def _extract_ref_answer_from_conversation(conversation):
    """
    Helper to pull the ground-truth numeric answer string from a GSM8K conversation.
    Mirrors logic in GSM8K.evaluate.
    """
    ...
    return extract_answer(last_text_part)


def _format_reward(generated_text):
    """
    Reward that encourages well-formed GSM8K-style answers:
    - must contain a '#### <number>' pattern that extract_answer can parse.
    """
...
def _steps_reward(generated_text):
    """
    Reward that encourages a reasonable number of calculator/tool calls.
    From Part 3 plots, accuracy peaks around 1–2 reasoning steps and falls off
    for 0 and for very long chains.
    """
    # Count calculator-style tool calls of the form << ... >>
    step_count = generated_text.count("<<")
    # Shape: peak at 2 steps, degrade linearly as we move away, floor at 0.
    ideal_steps = 2.0
    # Every extra step away from 2 reduces reward by 0.25
...
def _close_arithmetic_reward(conversation, generated_text):
    """
    Partial credit when the numeric answer is close to correct.
    This targets 'close_arithmetic' errors from Part 3.
    """
    ref_str = _extract_ref_answer_from_conversation(conversation)
    pred_str = extract_answer(generated_text)
    if ref_str is None or pred_str is None:
        return 0.0
    try:
    ...
    if math.isclose(ref_val, pred_val, rel_tol=0.0, abs_tol=0.0):
        # Exact match should already be rewarded by the base correctness reward
        return 0.0

    diff = abs(ref_val - pred_val)
    # Use relative error when the reference magnitude is not tiny, otherwise absolute.
    if abs(ref_val) > 1.0:
        rel_err = diff / abs(ref_val)
        if rel_err <= 0.05:
        ...
        # Small numbers: simple absolute thresholds
        if diff <= 0.5:
            return 0.7
        if diff <= 1.0:
            return 0.4
...
def compute_combined_reward(train_task, conversation, generated_text):
    """
    Compute base correctness reward plus additional shaped rewards:
    - correctness: original GSM8K 0/1 reward
    - format: encourages '#### <number>' extraction
    - steps: encourages 1–3 reasoning/tool steps
    - close_arithmetic: partial credit when numerically close

    Returns:
        total_reward (float),
        components (dict) with individual reward components (for logging).
    """
    base_correct = float(train_task.reward(conversation, generated_text))
    r_format = _format_reward(generated_text)
    r_steps = _steps_reward(generated_text)
    r_close = _close_arithmetic_reward(conversation, generated_text)

    total = (
        args.w_correct * base_correct
        + args.w_format * r_format
        + args.w_steps * r_steps
        + args.w_close * r_close
    )
    components = {
        "r_correct": base_correct,
        "r_format": r_format,
        "r_steps": r_steps,
        "r_close": r_close,
        "r_total": total,
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        # Calculate the rewards for each sample (combined reward + components)
        rewards = []
        ...
        for sample_tokens in generated_token_sequences:
            ...
            # Calculate the combined reward and keep components for logging
            reward, components = compute_combined_reward(train_task, conversation, generated_text)
...
for step in range(num_steps):
    ...
    if master_process and ((step > 0 and step % args.save_every == 0) or step == num_steps - 1):
        ...
        output_dirname = save_tag if save_tag else f"d{depth}"
