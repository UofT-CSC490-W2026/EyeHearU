...
Individual stages (if you want to re-run one step):
    ...
    modal run nanochat_modal.py::stage_pretrain_swiglu   # SwiGLU ablation
    modal run nanochat_modal.py::stage_pretrain_rmsnorm # Learnable RMSNorm ablation
...
def stage_pretrain_rmsnorm(
    depth: int = DEPTH,
    device_batch_size: int = DEVICE_BATCH_SIZE,
    wandb_run: str = "picochat_rmsnorm",
) -> None:
    """
    Pretrain with learnable RMSNorm scale γ (ablation). Requires gpt_rmsnorm.py and
    scripts/base_train_rmsnorm.py to be in the nanochat/ copy before image build.
    Same as stage_pretrain but runs scripts.base_train_rmsnorm with model-tag d12_rmsnorm.
    """
    _setup_cache()
    print("Resetting training report...")
    _python("nanochat.report", ["reset"])
    print(
        f"Starting RMSNorm pretraining: depth={depth}, "
        f"device_batch_size={device_batch_size}, nproc={_N_PRETRAIN_GPUS}, run={wandb_run}"
    )
    model_tag = f"d{depth}_rmsnorm"
    _torchrun(
        "scripts.base_train_rmsnorm",
        [
            f"--depth={depth}",
            f"--device-batch-size={device_batch_size}",
            f"--run={wandb_run}",
    ...
    volume.commit()
    print("RMSNorm pretraining complete.")
...
def stage_post_pretrain_eval_rmsnorm(model_tag: str = "d12_rmsnorm") -> None:
    """
    Evaluate the RMSNorm model checkpoint. Requires gpt_rmsnorm.py and
    scripts/base_eval_rmsnorm.py in nanochat before image build.
    Runs base_eval_rmsnorm (loads GPTRMSNorm) instead of base_eval.
    """
    _setup_cache()
    ...
    print(f"Running base_eval_rmsnorm for model_tag={model_tag} (val bpb + CORE metric, ~20-40 min)...")
    _torchrun(
        "scripts.base_eval_rmsnorm",
        ["--device-batch-size=16", f"--model-tag={model_tag}"],
        nproc=_N_PRETRAIN_GPUS,
    )

    volume.commit()
    print("Post-pretrain eval (RMSNorm) complete.")
...
# =============================================================================
# A4 PART 2 — TASK 1: SFT + RL with original configuration
# =============================================================================
...
def stage_sft_task1(wandb_run: str = WANDB_RUN_TASK1_SFT) -> None:
    """
    ...
    Run:
        modal run nanochat_modal.py::stage_sft_task1
    """
    _setup_cache()
...
def stage_rl_task1(wandb_run: str = WANDB_RUN_TASK1_RL) -> None:
    ...
    Run:
        modal run nanochat_modal.py::stage_rl_task1
    """
    _setup_cache()
